...
class ResidentModel:
    ...
    def move_resident(self, resident_id, new_household_id):
        self.cursor.execute(
            "UPDATE admin SET HouseholdID=%s WHERE RefId=%s",
            (new_household_id, resident_id)
        )
        self.conn.commit()
...
def dashboard():
    ...
    search_resident = request.args.get("search_resident", "").strip()  # new for residents

    # Zones
    all_zones = user_ctrl.zone_counts()  # total zones for the card
    zones = all_zones.copy()
    if search_zone:
        zones = {z: count for z, count in zones.items() if search_zone.lower() in z.lower()}

    # Households
    all_households = user_ctrl.get_households()
    households = all_households.copy()
    if search_household:
        households = {k: v for k, v in households.items() if search_household.lower() in k.lower()}

    # Residents
    all_residents = user_ctrl.get_all_residents()  # for total card
    residents = all_residents.copy()
    if search_resident:
        residents = [
            r for r in residents
            if search_resident.lower() in r["RefId"].lower()
               or search_resident.lower() in r["Name"].lower()
               or search_resident.lower() in r["LastName"].lower()
        ]
...
@user_bp.route("/move_resident", methods=["POST"])
@user_login_required
def move_resident():
    resident_id = request.form.get("resident_id")
    to_hh = request.form.get("to_hh")

    if not resident_id or not to_hh:
        flash("Invalid move request", "danger")
        return redirect(url_for("user.manage_households"))

    # Fetch current household of the resident
    resident = user_ctrl.get_resident(resident_id)
    if resident.get("HouseholdID") == to_hh:
        flash(f"Resident {resident['Name']} is already in {to_hh}!", "danger")
        return redirect(url_for("user.manage_households"))

    # Perform move
    user_ctrl.move_resident(resident_id, to_hh)
    flash(f"Resident {resident['Name']} moved to {to_hh} successfully!", "success")
    return redirect(url_for("user.manage_households"))
