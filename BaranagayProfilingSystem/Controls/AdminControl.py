...
def manage_residents():
    if request.method == "POST":
        ...
        try:
            ...
            flash("Invalid age! Age must be a non-negative number.", "danger")
            return redirect("/admin/manage_residents")   # <-- ALWAYS return here

        # --- Continue if age is valid ---
    ...
    # --- GET Request: Show list of residents ---
...
def update_resident(ref_id):
    updated_age_input = request.form.get("age", "").strip()
    try:
        updated_age = int(updated_age_input)
        if updated_age < 0:
            raise ValueError("Age cannot be negative.")
    except ValueError:
        flash("Invalid age! Age must be a non-negative number.", "danger")
...
def settings():
    ...
    current_username = admin_ctrl.get_username() or "Admin"
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if logo_file and logo_file.filename != "":
                logo_file.save("static/barangaypic.png")
    ...
    return render_admin("settings", system_info=system_info)
